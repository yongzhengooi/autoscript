...
def add_port_column(sheet):
	...
	if "Port" not in headers:
		...
		sheet.cell(row=1, column=scope_index-1).value = "Port"
		for row in range(2, sheet.max_row + 1):
    			sheet.cell(row=row, column=scope_index-1).value = ""
				...
				for port in scope_list:
					if "TCP" in port or "UDP" in port:
						...
						for list_index in range (0,len(port_scope)):
							...
							mod_scope(newexcel,current_row,scope_index,port_scope[list_index])
							...
							if re.search(r"[:,]\s*(\d+)", port):
								...
								if match:
									...
									for h_port in match:
										...
										create_row(newexcel,row_values,current_row)
										mod_scope(newexcel,current_row,scope_index,new_url)
