...
def add_market_column(sheet,filename):
	headers = [cell.value for cell in sheet[1]]
	if "amway" in filename.lower() and "Market" not in headers:
		country=filename.split("_")
		sheet.insert_cols(scope_index-1)
		sheet.cell(row=1, column=scope_index-1).value = "Market"
		for row in range(2, sheet.max_row + 1):
    			sheet.cell(row=row, column=scope_index-1).value = country[1] 
...
def main(workbook_location):
	...
	if verfied:
		...
		for i in range(2,last_row +1):
			...
			if len(scope_list) >=2:
				for list_index in range (0,len(scope_list)):
					create_row(newexcel,row_values,current_row)
					mod_scope(newexcel,current_row,scope_index,scope_list[list_index])
					current_row+=1
			else:
		...
		add_market_column(newexcel,file_path)
